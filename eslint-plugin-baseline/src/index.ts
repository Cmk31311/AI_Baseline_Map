...
export const baselineFeatures: Record<string, BaselineRule> = {
  // CSS Features
  'css-grid': {
...
export const cssPatterns: Record<string, RegExp> = {
...
export const jsPatterns: Record<string, RegExp> = {
      ...
      create(context: any) {
        ...
        return {
          Program(node: any) {
            ...
            const patterns = isCSS ? cssPatterns : jsPatterns;
            ...
            Object.entries(patterns).forEach(([featureId, pattern]) => {
              ...
              if (baselineLevel === 'high' && feature.baseline !== 'high') return;
              if (baselineLevel === 'low' && feature.baseline === false) return;
              ...
              if (matches) {
                matches.forEach((match: string) => {
                  ...
                  let messageData: any = { message: feature.message };
